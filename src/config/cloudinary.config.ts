...
export const generateUniqueFileName = (originalName: string) => {
    ...
    return { uniqueName, folder, extension };
};
...
export const uploadFileToCloudinary = async (
    ...
    return new Promise((resolve, reject) => {
        cloudinary.uploader.upload_stream(
            ...
            (error, result) => {
                if(error){
                    return reject(new ApiError(status.INTERNAL_SERVER_ERROR, "Failed to upload file to Cloudinary"));
                }
...
export const deleteFileFromCloudinary = async (url : string) => {
    ...
    try {
        ...
        throw new ApiError(status.INTERNAL_SERVER_ERROR, "Failed to delete file from Cloudinary");
    }
}
